local Translations = {
    ...
    info = {
        ...
        wep_unknown = 'Unknown',
        respawn_txt = 'ВОСКРЕШЕНИЕ ЧЕРЕЗ: ~r~%{deathtime}~s~ СЕКУНД',
        respawn_revive = 'HOLD [~r~E~s~] FOR %{holdtime} SECONDS TO RESPAWN FOR $~r~%{cost}~s~',
        bleed_out = 'ВЫ ИСТЕКАЕТЕ КРОВЬЮ И УМРЕТЕ ЧЕРЕЗ ~r~%{time}~s~ СЕКУНД',
        bleed_out_help = 'ВЫ  УМРЕТЕ ЧЕРЕЗ: ~r~%{time}~s~ СЕКУНД, НО ВАС МОЖНО СПАСТИ',
        weapon_remove = 'ПОЛИЦЕЙСКИЙ ИЗЬЯЛ ВАШЕ ОРУЖИЕ: ~r~%{weapon}~s~ ',
        request_help = 'НАЖМИТЕ [~r~G~s~] ДЛЯ ВЫЗОВА ПОМОЩИ',
        help_requested = 'ПОМОЩЬ УЖЕ В ДОРОГЕ',
        help_requested_nodoctors = 'СПАСАТЕЛИ ОПОВЕЩЕНЫ, НО СВОБОДНЫХ ДОКТОРОВ НЕТ',
        button_to_survive = '[~r~E~s~] - ПРОДОЛЖИТЬ',
